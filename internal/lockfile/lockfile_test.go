...
func TestWriteAndRead_Metadata(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "test.lock")
	...
	if len(got.Metadata) != 3 {
		t.Errorf("Metadata length = %d, want 3", len(got.Metadata))
	}
	if got.Metadata["result_path"] != "/tmp/test-output.txt" {
		t.Errorf("Metadata[result_path] = %q, want %q", got.Metadata["result_path"], "/tmp/test-output.txt")
	}
	if got.Metadata["commit"] != "abc123" {
		t.Errorf("Metadata[commit] = %q, want %q", got.Metadata["commit"], "abc123")
	}
	if got.Metadata["exit_code"] != "0" {
		t.Errorf("Metadata[exit_code] = %q, want %q", got.Metadata["exit_code"], "0")
	}
}

func TestWriteOmitsMetadata_WhenNilOrEmpty(t *testing.T) {
	tests := []struct {
		name     string
		metadata map[string]string
	}{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			dir := t.TempDir()
			path := filepath.Join(dir, "nometa.lock")
			...
			if _, exists := raw["metadata"]; exists {
				t.Error("metadata should be omitted when nil or empty (omitempty)")
			}
		})
	}
}

func TestRead_BackwardCompat_NoMetadata(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "old.lock")
  ...
  "ttl_sec": 300
	...
	got, err := Read(path)
	if err != nil {
		t.Fatalf("Read() should accept lockfile without metadata, got error: %v", err)
	}

	if got.Metadata != nil {
		t.Errorf("Metadata should be nil for old lockfile, got %v", got.Metadata)
	}
	if got.Name != "test" {
		t.Errorf("Name = %q, want %q", got.Name, "test")
	}
