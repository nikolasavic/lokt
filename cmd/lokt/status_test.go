...
func TestStatus_JSON_WithMetadata(t *testing.T) {
	_, locksDir := setupTestRoot(t)

	hostname, _ := os.Hostname()
	writeLockJSON(t, locksDir, "cached.json", &lockfile.Lock{
		Name:       "cached",
		Owner:      "agent",
		Host:       hostname,
		PID:        1,
	...
	stdout, _, code := captureCmd(cmdStatus, []string{"--json", "cached"})
	if code != ExitOK {
		t.Errorf("expected exit %d, got %d", ExitOK, code)
	}
	...
	if out.Metadata["result_path"] != "/tmp/result.txt" {
		t.Errorf("expected result_path=/tmp/result.txt, got %q", out.Metadata["result_path"])
	}
	if out.Metadata["commit"] != "abc123" {
		t.Errorf("expected commit=abc123, got %q", out.Metadata["commit"])
	}
}

func TestStatus_JSON_NoMetadata(t *testing.T) {
	_, locksDir := setupTestRoot(t)

	hostname, _ := os.Hostname()
	writeLockJSON(t, locksDir, "nometa.json", &lockfile.Lock{
		Name:       "nometa",
		Owner:      "agent",
		Host:       hostname,
		PID:        1,
	...
	stdout, _, code := captureCmd(cmdStatus, []string{"--json", "nometa"})
	if code != ExitOK {
		t.Errorf("expected exit %d, got %d", ExitOK, code)
	}

	// Metadata should be omitted from JSON (not an empty object)
	var raw map[string]json.RawMessage
	if err := json.Unmarshal([]byte(stdout), &raw); err != nil {
		t.Fatalf("invalid JSON: %v\noutput: %s", err, stdout)
	}
	if _, exists := raw["metadata"]; exists {
		t.Error("metadata should be omitted when not set")
	}
}
...
func TestGetMeta_Success(t *testing.T) {
	...
	writeLockJSON(t, locksDir, "result.json", &lockfile.Lock{
		Name:  "result",
		Owner: "agent",
		Metadata: map[string]string{
			"path":      "/tmp/out.txt",
	...
	stdout, stderr, code := captureCmd(cmdGetMeta, []string{"result", "path"})
	if code != ExitOK {
		t.Fatalf("expected exit %d, got %d; stderr: %s", ExitOK, code, stderr)
	}
	if strings.TrimSpace(stdout) != "/tmp/out.txt" {
		t.Errorf("expected '/tmp/out.txt', got %q", stdout)
	}
}

func TestGetMeta_EmptyValue(t *testing.T) {
	_, locksDir := setupTestRoot(t)

	writeLockJSON(t, locksDir, "empty.json", &lockfile.Lock{
		Name:  "empty",
		Owner: "agent",
		Metadata: map[string]string{
			"key": "",
	...
	stdout, _, code := captureCmd(cmdGetMeta, []string{"empty", "key"})
	if code != ExitOK {
		t.Fatalf("expected exit %d for empty value, got %d", ExitOK, code)
	}
	// Empty value should be returned (just newline from Println)
	if stdout != "\n" {
		t.Errorf("expected empty string (newline), got %q", stdout)
	}
}

func TestGetMeta_LockNotFound(t *testing.T) {
	setupTestRoot(t)

	_, stderr, code := captureCmd(cmdGetMeta, []string{"nonexistent", "key"})
	if code != ExitNotFound {
		t.Errorf("expected exit %d, got %d", ExitNotFound, code)
	}
	if !strings.Contains(stderr, "not found") {
...
func TestGetMeta_KeyNotFound(t *testing.T) {
	_, locksDir := setupTestRoot(t)

	writeLockJSON(t, locksDir, "hasother.json", &lockfile.Lock{
		Name:  "hasother",
		Owner: "agent",
		Metadata: map[string]string{
			"other": "value",
	...
	_, stderr, code := captureCmd(cmdGetMeta, []string{"hasother", "missing"})
	if code != ExitError {
		t.Errorf("expected exit %d for missing key, got %d", ExitError, code)
	}
	if !strings.Contains(stderr, "not found") {
		t.Errorf("expected 'not found' in stderr, got: %s", stderr)
	}
...
func TestGetMeta_NoMetadata(t *testing.T) {
	_, locksDir := setupTestRoot(t)
	...
	_, stderr, code := captureCmd(cmdGetMeta, []string{"nometa", "key"})
	if code != ExitError {
		t.Errorf("expected exit %d for lock without metadata, got %d", ExitError, code)
	}
	if !strings.Contains(stderr, "no metadata") {
		t.Errorf("expected 'no metadata' in stderr, got: %s", stderr)
	}
}

func TestGetMeta_Usage(t *testing.T) {
	setupTestRoot(t)
	...
	tests := []struct {
		...
		{"no args", nil},
		{"one arg", []string{"lock"}},
		{"three args", []string{"lock", "key", "extra"}},
	}

	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			_, stderr, code := captureCmd(cmdGetMeta, tc.args)
			if code != ExitUsage {
